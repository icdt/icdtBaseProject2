...
angular.module('app', []).controller('EditicdtdataCtrl', [
    ...
    function ($scope, $state, $stateParams, $timeout, $rootScope, GlobalService, DatetimeService, OptionService, icdtdatasFactory) {
        ...
        $scope.vm.isEdit = true;    // 是否為編輯頁面

        $scope.vm.singleObj = {};   // 宣告編輯物件
        $scope.vm.singleObj = GlobalService.selectedObj;    // 取得列表所選取的物件
        editor.setData($scope.vm.singleObj.CKeditorContent);
        $scope.vm.singleObj.AutoComplete = GlobalService.selectedObj.AutoComplete;
        ...
        $scope.viewAction.save = function () {
            ...
            if ($scope.signup_form.$valid) {
                ...
                icdtdatasFactory.update($scope.vm.singleObj).success(function (data) {
                    alert("儲存成功");
        ...
        // 刪除
        $scope.viewAction.remove = function () {
            icdtdatasFactory.remove($scope.vm.singleObj).success(function (data) {
                alert("刪除成功");
                $state.go('m.CRUD.list');
            }).error(function (err) {
                alert("刪除失敗");
